// select all and uncomment first.....then check code
...
@Component({
    ...
    template: `
           ...
           <!-- <div (click)="onDivClicked()"> -->
            ...
            <!-- <input (keyup)="onKeyUp($event)"/> -->          <!-- traditional method -->
            <!-- <input (keyup.enter)="onKeyUp()"/> -->          <!-- newer easier angular approach to the same problem -->


            <!-- <input #email (keyup.enter)="onKeyUp(email.value)" />   -->
            <!-- template variables -->
            <!-- <input [(ngModel)]="email" (keyup.enter)="onKeyUp()" /> -->    <!-- 2 way binding angular method...check email and this.email implementation here -->
            `
})
...
export class CoursesComponent {
    ...
    // onKeyUp($event) {
    //     if($event.keyCode === 13) console.log("ENTER was pressed") //traditional approach..13 here is enter keycode
    // }
    ...
    // onKeyUp(email) {
    //     console.log(email);         // but this is not the right way to keep passing email..we shld make use of OOPS here.. use ecapsulation of objects..see below method
    // }           
    
    email = "me@example.com";      //component encapsulates the data and html...here email is used to encapsulate the data

    onKeyUp() {     //keyup represents logic behind the view
        console.log(this.email);         // cleaner..easier to understand and implement...
    }



    /*     constructor(service: CoursesService) {
            // let service = new CoursesService();   //init the service 
