...
var sslcertificate  = {
  key: fs.readFileSync('/etc/ssl/secad-s19-addagudurua1.key'),
  cert: fs.readFileSync('/etc/ssl/secad-s19-addagudurua1.crt')
};
...
socketio.on('connection', function (socketclient) {
  console.log("A new socket.IO client is connected: "+ socketclient.client.conn.remoteAddress+":"+socketclient.id);
socketclient.on("message", (data) => {
  console.log("Received Data: " +data);
  socketio.emit("message",data);    //broadcast to all connected clients
});
socketclient.on('typing',function() {
  console.log("Someone is typing...");
  socketio.emit("typing");            //broadcast to all connected clients
});
